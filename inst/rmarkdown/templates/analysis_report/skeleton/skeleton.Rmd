...
library(repmis)
LoadandCite(pkgs = c("ggplot2", "dplyr", "tidyr", "readr", "purrr", "tibble", "stringr", "forcats", "lubridate", "car", "ProjectTemplate", "repmis", "biometrics", "knitr", "kableExtra", "broom", "GGally", "captioner", "jtools"),
...
m4 <- lm(Sepal.Length ~ Sepal.Width + Petal.Length + Species, data = iris)
...
```{r influence_plot, out.extra = "figure", echo = F, warning = F, message = F, comment = NA}
influencePlot(m4, id.method = "identify", main = "Influence Plot", sub = "Circle size is proportial to Cook's Distance" )

```

### Linearity of Relationships

```{r added_variable_plot, out.extra = "figure", echo = F, warning = F, message = F}
avPlots(m4)

```

```{r nonlinearity_crPlots, out.extra = "figure", echo = F, warning = F, message = F}
crPlots(m4) # component + residual plot 

```

### Consitancy of Error Variance (homoscedasticity)

```{r homoscedasticity_test, echo = F, comment = NA}
ncvTest(m4) # non-constant error variance test

```

```{r homoscedasticity_plot, out.extra = "figure", echo = F, warning = F, message = F, comment = NA}
...
```{r qqplot, out.extra = "figure", echo = F, warning = F, message = F, comment = NA}
qqPlot(m4, main="QQ Plot")

```

```{r residuals_distribution, out.extra = "figure", echo = F, warning = F, message = F}
sresid <- MASS::studres(m4) 
...
vif(m4) %>% as_tibble() # variance inflation factors 
...
vif(m4) %>% as_tibble() %>% mutate_all(list(~sqrt(.) > 2)) # problem?
