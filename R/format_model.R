...
#' @importFrom dplyr select mutate mutate_at case_when
#' @importFrom broom tidy
#'
#' @examples
#' \dontrun{
...
format_model <- function(x, effects = NULL, conf.level = 0.95, dp = 2) {
  ...
  estimate <- std.error <- conf.low <- conf.high <- estimate_tmp <- conf.high_tmp <- conf.low_tmp <- text1 <- NULL

  colours_3pal <- c("#8F715B", "#F1931B", "#D6618F") # negative, not significant, positive

  tidy(x, effects = effects) %>%
    ...
    mutate(text1 = paste0(.data$estimate_tmp, " (", 100 * conf.level, "% CI: ", .data$conf.low_tmp, " to ", .data$conf.high_tmp, ")"),
